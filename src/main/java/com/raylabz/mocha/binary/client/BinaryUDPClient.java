 ...
 * @version 1.0.0
 */
public abstract class BinaryUDPClient extends BinaryClient {
...
//                            }
     ...
     */
    public BinaryUDPClient(String name, String ipAddress, int port) throws UnknownHostException, SocketException {
    ...
    public final void send(final byte[] data) {
        if (isConnected()) {
            try {
                DatagramPacket packet = new DatagramPacket(data, data.length, getAddress(), getPort());
                socket.send(packet);
            } catch (IOException e) {
