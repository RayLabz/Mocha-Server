 ...
 * @version 1.0.0
 */
public class BinaryTCPHandler implements Runnable {
     ...
     */
    private final Vector<BinaryTCPConnection> tcpConnections = new Vector<>();
     ...
     */
    public BinaryTCPHandler(int port, final BinaryTCPReceivable receivable) {
        this.port = port;
     ...
     */
    public BinaryTCPReceivable getReceivable() {
        return receivable;
    }
     ...
     */
    public Vector<BinaryTCPConnection> getTcpConnections() {
     ...
     */
    void removeTCPConnection(BinaryTCPConnection tcpConnection) {
    ...
    void removeTCPConnectionsAndThreads() {
        try {
            for (BinaryTCPConnection connection : tcpConnections) {
     ...
     */
    public void multicast(byte[] data, ArrayList<InetAddress> ipAddresses) {
        if (isEnabled()) {
            for (BinaryTCPConnection connection : tcpConnections) {
        ...
        try {
            ...
            while (isEnabled()) {
                ...
                if (server.getBannedAddresses().contains(socket.getInetAddress())) {
                    ...
                    BinaryTCPConnection tcpConnection = new BinaryTCPConnection(socket, receivable);
            ...
            if (!isEnabled()) {
                System.out.println("Stopped listening to TCP port " + port + "." + se.getMessage());
                Logger.logInfo("Stopped listening to TCP port " + port + "." + se.getMessage());
            }
        } catch (IOException e) {
            System.err.println("[TCP " + port + "]" + "Error: " + e.getMessage());
            Logger.logError(e.getMessage());
